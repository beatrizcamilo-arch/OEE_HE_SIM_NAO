    ...
    SELECT 
        ...
        CASE 
            WHEN rti.workTeamName = 'TA' AND (
                DATEDIFF(SECOND, DATEADD(HOUR, 14, CAST(CAST(rtf.stoptime AS DATE) AS DATETIME)), rtf.stoptime) > 0
                OR DATEDIFF(SECOND, rti.starttime, DATEADD(HOUR, 6, CAST(rti.starttime AS DATE))) > 0
            ) THEN 'Sim'
            WHEN rti.workTeamName = 'TB' AND (
                DATEDIFF(SECOND, DATEADD(HOUR, 22, CAST(CAST(rtf.stoptime AS DATE) AS DATETIME)), rtf.stoptime) > 0
                OR DATEDIFF(SECOND, rti.starttime, DATEADD(HOUR, 14, CAST(rti.starttime AS DATE))) > 0
            ) THEN 'Sim'
            WHEN rti.workTeamName = 'TC' AND (
                DATEDIFF(SECOND, DATEADD(HOUR, 06, CAST(CAST(rtf.stoptime AS DATE) AS DATETIME)), 
